...
export class WhereValidator {
    static getSchema<Row extends object>(
        ...
        onlyWhereKeys?: (keyof Row)[]): SchemaValidator<WhereParams<Row>> {
        return this.getWhereSchemaByKeys(rowSchema, onlyWhereKeys)
    }
    
    private static getWhereSchemaByKeys<Row extends object>(rowSchema: SchemaValidator<Row>, onlyWhereKeys?: (keyof Row)[]): SchemaValidator<WhereParams<Row>> {
...
//@ts-ignore
    ...
    private static checkIsStringType(type: SchemaValidatorType): boolean {
        return type === 'string' ||
            type === 'literal' ||
            type === 'enum'
    ...
    private static checkIsNumberType(type: SchemaValidatorType): boolean {
        return type === 'number' ||
            type === 'booleanNum'
        
    }
    
    private static checkIsDateType(type: SchemaValidatorType): boolean {
        return type === 'date'
    }
