...
import sequelize, {FindAttributeOptions, Op, Order, Sequelize, WhereOptions} from 'sequelize'
...
export class DbConnectionSqlModelQueryBuilder<
    Row extends object,
...
> {
    ...
    public buildAttributes({
        ...
        const res: FindAttributeOptions = []
    ...
    public buildWhere<Row extends object>({
                                              where,
                                              filters,
                                          }: {
        ...
        return {
            ...this.parseWhereParamsToOpSequelize(where),
            ...this.parseFiltersToOpSequelize(filters),
        }
    }
    
    public buildOrder<Aggregates extends Record<string, SqlAggregate<Row>> = {}, >({
                                                                                       order,
                                                                                       include,
                                                                                       orderAggregate,
                                                                                   }: {
    ...
    private parseWhereParamsToOpSequelize<Row extends object>(where?: WhereParams<Row> | undefined): WhereOpParamsSequelize<Row> {
        if (!where) {
            return {}
        }
        
        const filters: WhereOpParamsSequelize<Row> = {}
        
        for (const k in where) {
            const key = k as keyof Row | WhereKeys<Row>
            //@ts-ignore
            ...
            if (arr?.length >= 2) {
                ...
                const targetKey: keyof Row = arr[0] as keyof Row
            ...
            const targetKey = key as keyof Row
    ...
    private parseFiltersToOpSequelize<Row extends object>(filters?: SqlFilters<Row>): WhereOpParamsSequelize<Row> {
        if (!filters) {
            return {}
        }
